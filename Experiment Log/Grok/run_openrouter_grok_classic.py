...
SERVER_PORT = 3000
CLASSIC_SITE_DIR = Path("Classic site")
SERVER_URL = f"http://127.0.0.1:{SERVER_PORT}/"
MODEL_NAME = "x-ai/grok-4-fast"
LOG_BASE_DIR = Path("log-baseline-openrouter-x-ai-grok-4-fast")
...
def start_classic_server() -> subprocess.Popen:
    if not CLASSIC_SITE_DIR.exists():
        raise RuntimeError(f"Missing site directory: {CLASSIC_SITE_DIR}")

    kill_listeners_on_port(SERVER_PORT)

    process = subprocess.Popen(
        ["python", "-m", "http.server", str(SERVER_PORT)],
        cwd=str(CLASSIC_SITE_DIR),
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
    ...
    if process.poll() is not None:
        raise RuntimeError(
            f"Classic server process exited immediately (likely port conflict): {CLASSIC_SITE_DIR}"
        )
    try:
...
async def main():
    ...
    try:
        for task_name in tasks:
            task_file = Path(f"{task_name}.txt")
            ...
            for run in range(1, 11):
                ...
                try:
                    print("=" * 70)
                    print(f"Starting isolated Classic-site server for {task_name} | run: {run}/10 on {SERVER_URL}")
                    print("=" * 70)
                    server_process = start_classic_server()
                    ...
                    if server_process is not None:
                        print("=" * 70)
                        print(f"Stopping isolated Classic-site server for {task_name} | run: {run}/10")
                        print("=" * 70)
